...
bash /root/socks5-toolkit-v1.9.4/xshell-install.sh --port 31080
```
...
1. 在 Windows 下载本项目 `v1.9.4` 源码压缩包并解压。
2. 使用 Xshell 远程文件管理器或 Xftp，把整个目录上传到 `/root/socks5-toolkit-v1.9.4`。
3. 回到 `root@...#` 运行：
...
```bash
bash /root/socks5-toolkit-v1.9.4/xshell-install.sh --port 31080 --overwrite
```
...
不要重复这里的安装步骤。统一按照[安全升级到指定版本](upgrade.md)操作；升级器只接受已有
本项目节点，不会执行首次安装、降级或其他代理迁移。
