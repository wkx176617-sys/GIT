...
1. 在 Windows 下载本项目 `v1.9.0` 源码压缩包并解压。
2. 使用 Xshell 远程文件管理器或 Xftp，把整个目录上传到 `/root/socks5-toolkit-v1.9.0`。
3. 回到 `root@...#` 运行：
...
```bash
bash /root/socks5-toolkit-v1.9.0/xshell-install.sh --port 31080 --overwrite
```
