...
当前推荐工具：`v1.12.0`；GOST：`3.2.6`。新节点推荐 Ubuntu 24.04 LTS `amd64`，已有
...
首次搭建只沿这条主线操作。安装会自动质检并开启核心 BBR + FQ，不需要额外命令；旧节点迁移、
升级、回退和故障维修都在首次成功后按需查看。
...
平台教程会先识别服务器状态：未安装时全新搭建，已有本项目节点时兼容升级，其他或未知程序
停止并给出中文说明。显示完成后，SSH 登录 VPS，逐行运行：
