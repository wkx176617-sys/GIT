...
| `BBR_INACTIVE` | 核心策略要求 BBR + FQ，但当前未完整生效 | 禁止自动维修；进入性能专题 |
...
IP、DNS、WebRTC 和时区都正确时，不要把“隔离正确”误当成“线路健康”。先在相同窗口、相同
时间段比较下载、上传、延迟、抖动和丢包，再用另一个客户端交叉验证。多个客户端同时慢，优先
怀疑共同线路、VPS 出口或供应商上游；只有一个客户端慢，才优先检查该客户端。

运行 `socksctl bbr health` 只检查核心加速是否生效，不会证明线路本身优质。完整判断、VPN
中转与最终出口的区别、启用前后对比方法见[网络性能与核心 BBR](network-performance.md)。
