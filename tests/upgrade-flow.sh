...
awk -F= '$1 == "TOOL_VERSION" {print "TOOL_VERSION=1.9.5"; found=1; next} \
  {print} END {if (!found) print "TOOL_VERSION=1.9.5"}' \
...
upgrade_output=$(printf 'UPGRADE-v1.9.5\n' | env "${upgrade_environment[@]}" \
  "$project_dir/scripts/socks-upgrade" v1.9.5 2>&1)
after_identity=$(awk -F= '$1 != "TOOL_VERSION" {print}' "$config_dir/node.env")

grep -Fq '升级只读检查通过' <<<"$upgrade_output"
grep -Fq '安全升级完成：早期版本 → v1.9.5' <<<"$upgrade_output"
...
if timeout_output=$(env "${upgrade_environment[@]}" SOCKS_TIMEOUT_BIN="$timeout_failure" \
  "$project_dir/scripts/socks-upgrade" --check v1.9.5 2>&1); then
